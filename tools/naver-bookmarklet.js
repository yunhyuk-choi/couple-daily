...
(function () {
  ...
  function scanFrames(win, depth) {
    ...
    for (var i = 0; i < frames.length; i++) {
      ...
      var cw = null;
      try { cw = fr.contentWindow; } catch (e) { diagOnce('noacc:' + src, 'iframe 접근불가: ' + src); continue; }
      if (!cw) continue;
      var acc = true;
      try { void cw.location.href; } catch (e) { acc = false; }
      if (!acc) { diagOnce('xo:' + src, 'iframe cross-origin 스킵: ' + src); continue; }
  ...
  function processNative(u) {
    ...
    if (!ius || typeof ius.uploadImagesFromFiles !== 'function') {
      diag('_imageUploadService.uploadImagesFromFiles 없음 → execCommand 폴백');
      runCommandFallback();
      return;
    }
    diag('네이티브 업로더 사용 — _imageUploadService.uploadImagesFromFiles');
    ...
    function uploadNative() {
      ...
      try { ret = ius.uploadImagesFromFiles(files); }
      catch (e) { diag('uploadImagesFromFiles throw: ' + (e && e.message ? e.message : e)); ret = undefined; }
      diag('uploadImagesFromFiles 반환 typeof=' + typeof ret + ' isPromise=' + (ret && typeof ret.then === 'function') + ' isArray=' + Array.isArray(ret));
      if (ret === undefined) {
        try {
          var srcList = (typeof ius.createSourceList === 'function')
            ? ius.createSourceList(files)
            : files.map(function (f, i) { return { id: 'cd' + i, source: f }; });
          ret = ius.uploadImages(srcList);
        } catch (e2) { return Promise.reject(new Error('uploadImages throw: ' + (e2 && e2.message ? e2.message : e2))); }
      }
      return normalize(ret);
    }

    setStatus(u, '네이버 인프라 업로드 중… (0/' + files.length + ')');
    uploadNative().then(function (results) {
      results = results || [];
      diag('업로드 결과 개수=' + results.length);
      if (!results.length) diag('업로드 결과 비어있음');
      ...
      for (var r = 0; r < results.length; r++) {
        ...
        var url = resp.url || resp.src || resp.imageUrl || resp.originalUrl || (resp.image && resp.image.url) || '';
        ...
        if (!url && path) url = 'https://blogfiles.pstatic.net' + (String(path).charAt(0) === '/' ? '' : '/') + path + '?type=w1';
        diag('업로드결과[' + r + '] code=' + (code || '?') + ' url=' + url + ' path=' + path + ' ' + width + 'x' + height);
        if (r < fileSrcs.length && url) {
          metaByBlockSrc[imgKey(fileSrcs[r])] = { displayUrl: url, path: path, width: width, height: height, fileSize: fileSize, fileName: fileName };
          okCount++;
        }
      }
      if (!okCount) { diag('성공 업로드 0 → execCommand 폴백'); runCommandFallback(); return; }
      setStatus(u, '네이버 인프라 업로드 중… (' + okCount + '/' + files.length + ')');
      buildAndSet('native');
    }).catch(function (e) {
      diag('네이티브 업로드 실패: ' + (e && e.message ? e.message : e) + ' → execCommand 폴백');
      runCommandFallback();
    });
  }
