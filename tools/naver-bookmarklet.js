...
(function () {
  ...
  function getEditorInstance() {
    try {
      var w = findEditorFrame() || window, Sm = w.SmartEditor;
      if (!Sm) return null;
      var e = null;
  ...
  // 🅢 정식 삽입 — 에디터 '네이티브' 업로드 파이프라인(execCommand INSERT_IMAGE_FILES)에
  // 파일을 태워 진짜 내부이미지 컴포넌트(정상 멀티렌디션 src/라이브러리 등록)를 만들게
  // 하고, 그 결과 컴포넌트를 읽어와(readback) 우리 doc.blocks 순서로 재배치해
  // setDocumentData 한다. upphoto(getSessionKey/uploadOne) 경로는 쓰지 않는다.
  function processNative(u) {
    ...
    if (!ed._commandManager || typeof ed._commandManager.execCommand !== 'function' || !Sm || !Sm.COMMAND || !Sm.COMMAND.IMAGE) {
      diag('중단: _commandManager.execCommand / COMMAND.IMAGE 없음');
      setStatus(u, '에디터 명령 API를 못 찾았어 — 다시 시도해줘', true);
      return;
    }
    diag('execCommand 사용가능 · COMMAND.IMAGE=' + JSON.stringify(Sm.COMMAND.IMAGE));

    var imgs = Array.isArray(p.images) ? p.images : [];
    // dataUri → File, 순서 보존 + 각 파일이 대응하는 블록 __src를 병렬 배열로.
    var files = [], fileSrcs = [];
    for (var i = 0; i < imgs.length; i++) {
      var im = imgs[i];
      if (!im || !im.dataUri || !im.src) continue;
      var blob = dataUriToBlob(im.dataUri);
      var name = 'image' + i + '.jpg';
      var f;
      try { f = new File([blob], name, { type: blob.type || 'image/jpeg' }); }
      catch (e) { f = blob; try { f.name = name; } catch (e2) {} }
      files.push(f); fileSrcs.push(im.src);
    }
    diag('네이티브 업로드 대상 파일 ' + files.length + '장');
    ...
    function buildAndSet() {
      ...
      diag('컴포넌트 ' + comps.length + '개 생성 (네이티브 이미지 ' + got + '/' + files.length + ')');
      ...
      diag('정식 문서 삽입 완료 (네이티브 이미지 ' + got + '장)');
      setStatus(u, '정식 문서 삽입 완료 (네이티브 이미지 ' + got + '장) — 편집기 확인 후 발행');
      u.go.disabled = false;
    }

    if (!files.length) { buildAndSet(); return; }

    // 네이티브 업로드 호출 — 인자 형태를 순서대로 시도(던지지 않는 첫 형태 채택).
    var CMD = Sm.COMMAND.IMAGE.INSERT_IMAGE_FILES;
    var uploaded = false, usedShape = '';
    function tryShape(shape, arg) {
      if (uploaded) return;
      try { ed._commandManager.execCommand(CMD, arg); uploaded = true; usedShape = shape; diag('네이티브 업로드 호출 OK — shape=' + shape + ' cmd=' + CMD); }
      catch (e) { diag('업로드 호출 실패 shape=' + shape + ': ' + (e && e.message ? e.message : e)); }
    }
    ...
    if (!uploaded) {
      diag('단일 파일 루프 시도');
      try { files.forEach(function (f) { ed._commandManager.execCommand(CMD, [f]); }); uploaded = true; usedShape = 'loop[f]'; diag('네이티브 업로드 호출 OK — shape=loop[f]'); }
      catch (e) { diag('단일 파일 루프 실패: ' + (e && e.message ? e.message : e)); }
    }
    if (!uploaded) { setStatus(u, '네이티브 업로드 호출 실패 — 진단 로그 확인', true); u.go.disabled = false; return; }

    // 완료 폴링 — 500ms 간격 최대 40s. new(=before에 없는) 이미지가
    // https://blogfiles.pstatic.net/ 로 채워지고 imageLoaded!==false 면 ready.
    ...
    var iv = setInterval(function () {
      ...
      try {
        if (ed._editingService && ed._editingService.isDocumentProcessing) {
          var pr = (typeof ed._editingService.isDocumentProcessing === 'function') ? ed._editingService.isDocumentProcessing() : ed._editingService.isDocumentProcessing;
          processing = processing || !!pr;
        }
      } catch (e) {}
      var ready = [];
      ...
      if ((ready.length >= files.length && !processing) || timeUp) {
        clearInterval(iv);
        diag('네이티브 업로드 완료 ' + ready.length + '/' + files.length + (timeUp ? ' (타임아웃)' : '') + ' processing=' + processing + ' shape=' + usedShape);
        if (ready.length < files.length) diag('경고: 예상보다 적게 도착 (' + ready.length + '/' + files.length + ') — 있는 것만 진행');
        // 업로드 순서 == 문서 등장 순서 == files 순서 로 매핑.
