  ...
  function processNative(u) {
    ...
    function extractMetas(results) {
      ...
      for (var r = 0; r < results.length; r++) {
        ...
        // 서빙 가능한 full URL 조립. rawUrl(<url>)은 이미 /fileName 을 포함한다.
        // path(<path>)는 fileName이 빠진 bare 라 직접 쓰면 404 → fileName 세그먼트를 붙인다.
        var full = '';
        if (rawUrl) {
          full = /^https?:/i.test(rawUrl) ? rawUrl : ('https://blogfiles.pstatic.net' + (String(rawUrl).charAt(0) === '/' ? '' : '/') + rawUrl);
        } else if (path) {
          full = 'https://blogfiles.pstatic.net' + (String(path).charAt(0) === '/' ? '' : '/') + path + '/' + fileName;
        }
        if (full && full.indexOf('?type=') === -1) full += (full.indexOf('?') === -1 ? '?' : '&') + 'type=w1';
        diag('업로드결과[' + r + '] code=' + (code || '?') + ' url=' + full + ' path=' + path + ' ' + width + 'x' + height);
        if (full) out.push({ idx: r, meta: { displayUrl: full, path: path, width: width, height: height, fileSize: fileSize, fileName: fileName } });
      }
      return out;
    }
