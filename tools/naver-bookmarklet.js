...
(function () {
  ...
  function buildComponents(doc, metaBySrc) {
    ...
    for (var b = 0; b < blocks.length; b++) {
      ...
      if (t === 'heading') {
        ...
        if (meta) comps.push(imageComp(meta));
        else diag('정식: 이미지 메타 없음 (src=' + (blk.__src || '') + ')');
      } else if (t === 'info') {
        var ip = [];
  ...
  // 🅢 정식 삽입 — 이미지를 upphoto에 올린 뒤, 네이티브 문서를 조립해 setDocumentData.
  function processNative(u) {
    ...
    var imgs = Array.isArray(p.images) ? p.images : [];
    var manual = (u.manualTa.value || '').trim(), mAuth = '', mApp = '';
    if (manual) { var ml = manual.split(/\r?\n/); mAuth = (ml[0] || '').trim(); mApp = (ml[1] || '').trim(); }
    u.go.disabled = true;
    setStatus(u, imgs.length ? '세션키 준비 중…' : '문서 구성 중…');
    var need = imgs.length ? getSessionKey(mAuth, mApp) : Promise.resolve(null);
    need.then(function (key) {
      var blogId = guessBlogId();
      var metaBySrc = {};
      var chain = Promise.resolve();
      imgs.forEach(function (img, i) {
        chain = chain.then(function () {
          ...
          var blob = dataUriToBlob(img.dataUri);
          return uploadOne(key, blogId, blob, i).then(function (meta) {
            metaBySrc[imgKey(img.src)] = meta;
          }).catch(function (e) {
            diag('정식 업로드[' + i + '] 실패: ' + (e && e.message ? e.message : e));
          });
        });
      });
      return chain.then(function () {
        var comps = buildComponents(p.doc, metaBySrc);
        diag('컴포넌트 ' + comps.length + '개 생성 (이미지 업로드 ' + Object.keys(metaBySrc).length + '/' + imgs.length + ')');
        var d = ed.getDocumentData();
        if (!d || !d.document) throw new Error('getDocumentData 형식 예상밖');
        d.document.components = comps;
        ed.setDocumentData(d);
        diag('setDocumentData 완료 — 편집기 확인');
        setStatus(u, '정식 문서 삽입 완료 — 편집기에서 확인 후 발행');
        u.go.disabled = false;
      });
    }).catch(function (e) {
      diag('정식 삽입 실패: ' + (e && e.message ? e.message : e));
