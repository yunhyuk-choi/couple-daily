 ...
 *   - 그래서 '요청'하지 않고, 네이버 에디터가 스스로 부르는 session-key '응답'을
 *     가로채(fetch/XMLHttpRequest 몽키패치) sessionKey를 뽑아 재사용한다(주 경로).
 *   - 추가로 에디터의 session-key '요청' 필드(메서드·URL·헤더)도 캡처해 오버레이에
 *     보여준다 — 폰엔 DevTools가 없어 이게 그 검사(inspection)를 대신한다(참고/폴백).
...
(function () {
  ...
  // 응답의 sessionKey는 window.__cdSK 에(주 재사용 경로), 요청 메타(메서드·URL·헤더)는
  // window.__cdReq 에 보관(진단/폴백 표시용). 둘 다 재클릭에도 유지, 콜백으로 최신
  // 오버레이 갱신(window.__cdSKcb / window.__cdReqCb).
  ...
  // 헤더 컬렉션(Headers 객체 / 배열 / 평범한 객체) → "name: value" 라인 배열.
  ...
  // 요청 메타 기록(진단 패널용). 쿠키/HttpOnly는 안 보이지만 authorization/x-* 같은
  // 커스텀 헤더와 쿼리는 보인다.
  ...
  function installInterceptor() {
    ...
    try {
      ...
      if (typeof of === 'function') {
        window.fetch = function () {
          ...
          // 요청 메타(메서드·URL·헤더) 캡처 — 진단 패널용(주 경로에 영향 없음).
          try {
    ...
    // XMLHttpRequest 래핑(open으로 메서드·URL 기억, setRequestHeader로 헤더 수집,
    // send에서 요청 메타 기록 + load에서 응답 sessionKey 추출)
    try {
      ...
      XMLHttpRequest.prototype.setRequestHeader = function (name, value) {
        try { if (!this.__cdHdrs) this.__cdHdrs = []; this.__cdHdrs.push(name + ': ' + value); }
        catch (e) {}
  ...
  var u = buildOverlay();
  installInterceptor();
  // 이 오버레이가 세션키/요청 캡처 시 갱신하도록 콜백 등록(재클릭이면 최신 오버레이가 받음).
  window.__cdSKcb = function () {
    setStatus(u, '세션키 확보 ✓ (이제 페이로드 붙여넣고 처리)');
  };
  window.__cdReqCb = function (info) {
    try { u.reqTa.value = formatReq(info); } catch (e) {}
  };
  if (window.__cdSK) {
    setStatus(u, '세션키 확보 ✓ (이제 페이로드 붙여넣고 처리)');
  } else {
